...
"""Head-to-head benchmark: raw-pointer vs. block-pointer log_softmax kernel.

Both kernels run in the same process against the same pre-allocated input.
...
Standalone script: not collected by pytest. Run with:
    python benchmarks/kernels/benchmark_logsoftmax_blockptr.py
"""
...
def bench_shape(n_rows: int, n_cols: int) -> dict:
    ...
    for r in range(N_ROUNDS):
        raw_first = (r % 2 == 0)
        t_raw, t_blk = _bench_one_round(x, raw_first)
        raw_ms.append(t_raw)
        blk_ms.append(t_blk)
